...
sub QueueStandardTemplateMemberList {
    ...
    if ($Param{QueueID}) {
        ...
        $CacheKey = "StandardTemplates::" . ($Param{UserID} || '-') . "::$Param{QueueID}::$TemplateTypes";
        ...
        # get std. templates
        my $SQL = "SELECT st.id, st.name, st.template_type, gt.name "
            . " FROM standard_template st, queue_standard_template qst, "
            . " standard_template_extended ex, groups_table gt "
            . " WHERE "
            . " qst.queue_id IN ("
            . $DBObject->Quote($Param{QueueID}, 'Integer')
            . ") AND "
            . " qst.standard_template_id = st.id AND "
            . " ex.standard_template_id = st.id AND "
            . " (ex.required_group = gt.id OR ex.required_group IS NULL) AND "
        ...
        my $SQL = "SELECT q.id, q.name"
            . " FROM queue q, queue_standard_template qst "
            . "WHERE "
