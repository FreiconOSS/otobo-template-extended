...
package Kernel::Modules::AgentTicketPhoneOld;
...
sub Run {
    ...
    if ( !$Self->{Subaction} || $Self->{Subaction} eq 'Created' ) {
        ...
        $Output .= $Self->_MaskPhoneNew(
            ...
            Services          => $Services,
            SLAs              => $SLAs,
            ### FREICON: get standard templates by type and filter
            ...
            #    QueueID => $Self->{QueueID} || '',
            #),
            StandardTemplates => $Self->_GetStandardTemplatesExtended(
                %GetParam,
                %ACLCompatGetParam,
                %SplitTicketParam,
                QueueID => $Self->{QueueID} || '',
    ...
    # create new ticket and article
        ...
        if ( $ElementChanged eq 'StandardTemplateID' ) {
            ...
            #my %PreTemplateData;
            #if ( IsPositiveInteger( $GetParam{PreStandardTemplateID} ) ) {
            #    %PreTemplateData = $Self->_StandardTemplateExtendedGet( ID => $GetParam{PreStandardTemplateID} );
            #}
            ### FREICON

            if ( IsPositiveInteger( $GetParam{StandardTemplateID} ) ) {

                #%TemplateData = $Kernel::OM->Get('Kernel::System::StandardTemplateExtended')->StandardTemplateGet( ID => $GetParam{StandardTemplateID} );
                ...
                if ( %TemplateData ) {
                    ...
                    if ( IsHashRefWithData($Config->{TemplateFieldsBehavior}) ) {
                        for my $Field (keys %{$Config->{TemplateFieldsBehavior}} ) {
                            ...
                            ### FREICON: previous standard template id for comparing subject
                            if ( $Field eq 'Subject' ) {
                                #$PreTemplateData{$Field} = '' unless ( defined $PreTemplateData{$Field} );
                                #                                print STDERR Dumper({ Field => $Field, Param => $GetParam{$Field}, Template => $TemplateData{$Field}, PreTemplate => $PreTemplateData{$Field}, });
                                if ( $GetParam{$Field} eq '' ) {
                                    $GetParam{$Field} = $TemplateData{$Field};
                                    #} elsif ( $PreTemplateData{$Field} eq $GetParam{$Field} ) {
                                    #    $GetParam{$Field} = $TemplateData{$Field};
                                } else {
                            ...
                            if ( $Field =~ m/^DynamicField_(.*)$/ ) {
                                ...
                                $DynamicFieldValues{$Name} = $TemplateData{$Field};
                            } elsif ( $Field eq 'Dest' ) {
                    ...
                    #if ( defined $TemplateData{QueueID} ) {
                    #    $GetParam{QueueID} = $TemplateData{QueueID};
                    #    $QueueID = $TemplateData{QueueID};
                    #    $GetParam{Dest} = $TemplateData{Dest};
                    #    $Dest = $TemplateData{Dest};
                    #}

                    #if (IsHashRefWithData($Config->{DynamicField})) {
                    #    for my $Name ( keys %{$Config->{DynamicField}} ) {
                    #        my $Field = 'DynamicField_' . $Name;
                    #        if ( defined $TemplateData{$Field} ) {
                    #            #if ( $Name eq 'KindOfReceipt' && $TemplateData{$Field} eq '' ) {
                    #            #    next;
                    #            #}
                    #            $GetParam{$Field} = $TemplateData{$Field};
                    #            $GetParam{'DynamicField'}->{$Field} = $TemplateData{$Field};
        ...
        #    QueueID => $QueueID || '',
        #);
        my $StandardTemplates = $Self->_GetStandardTemplatesExtended(
            %GetParam,
            %ACLCompatGetParam,
            QueueID => $QueueID || '',
        );
        ...
        if ( $ElementChanged eq 'StandardTemplateID' ) {
            ...
            ### FREICON: process dynamic fields of type "text"
            if ( IsHashRefWithData($Config->{TemplateFieldsBehavior}) ) {
                for my $Field (keys %{$Config->{TemplateFieldsBehavior}} ) {
                    if ( $Field =~ m/^DynamicField_(.*)$/ ) {
                        my $Name = $1;
                        my $DynamicFieldType = $DynamicFieldObject->DynamicFieldGet(Name => $Name)->{'FieldType'};
                        if ( $DynamicFieldType eq 'Text' ) {
                            push @TemplateAJAX, {
                                Name => $Field,
                                Data => $GetParam{$Field},
                            };
...
### FREICON: get standard templates by type and filter
sub _GetStandardTemplatesExtended {
    my ( $Self, %Param ) = @_;
    ...
    my $TemplateType = $Config->{TemplateType} || $Param{TemplateType} || 'Create';
    unless ( $TemplateType ) {
        return \%Templates;
    }

    my $FilterBy = $Config->{'FilterStandardTemplatesBy'};
    unless ( defined $FilterBy ) {
        $FilterBy = 'Queue';
    }

    if ( $FilterBy eq 'Queue' ) {
        ...
        # return empty hash if there are no templates for this screen
        return \%Templates if !IsHashRefWithData( $StandardTemplates{$TemplateType} );

        # return just the templates for this screen
        return $StandardTemplates{$TemplateType};

    } elsif ( $FilterBy eq 'Role' ) {

        my $Loaded = $Kernel::OM->Get('Kernel::System::Main')->Require(
            'Kernel::System::RoleStandardTemplate',
            Silent => 1,
        );

        if ( $Loaded ) {
            %Templates = $Kernel::OM->Get('Kernel::System::RoleStandardTemplate')->UserStandardTemplateMemberList(
                UserID => $Self->{UserID},
                Type   => $TemplateType,
            );
        }
        ...
        %Templates = $Kernel::OM->Get('Kernel::System::StandardTemplate')->StandardTemplateList(
            Valid => 1,
            Type  => $TemplateType,
        );
...
sub _MaskPhoneNew {
    ...
    if ( IsHashRefWithData( \%StandardTemplates ) ) {
        ...
        ### FREICON: Fields changeable by template
        $Param{StandardTemplateUpdateableStrg} = '';
        if ( IsHashRefWithData($Config->{TemplateFieldsBehavior}) ) {
            $Param{StandardTemplateUpdateableStrg} = join(', ', map { "'" . $_ . "'"} (keys %{$Config->{TemplateFieldsBehavior}} ) );
        }
        ### FREICON

        ### FREICON: different handling and positioning for template selection if templates are filtered by queue
        my $FilterStandardTemplatesBy = $Config->{FilterStandardTemplatesBy};
        if ( defined $FilterStandardTemplatesBy && $FilterStandardTemplatesBy eq '' ) {
            $FilterStandardTemplatesBy = 'None';
        }
        unless ( defined $FilterStandardTemplatesBy && $FilterStandardTemplatesBy =~ m/^(?:None|Queue|Role)$/ ) {
            $FilterStandardTemplatesBy = '';
        }
        ### FREICON
        ...
        $LayoutObject->Block(
            ...
            Name => 'StandardTemplate' . ( $FilterStandardTemplatesBy ? 'By' . $FilterStandardTemplatesBy : '' ),
            ### FREICON
            Data => {%Param},
        );
    }
...
### FREICON: TemplateData Mapping
sub _StandardTemplateExtendedGet {
    my ( $Self, %Param ) = @_;
    ...
    my %TemplateData = $Kernel::OM->Get('Kernel::System::StandardTemplateExtended')->StandardTemplateGet( ID => $Param{ID} );
        #print STDERR Dumper({%TemplateData});
    ...
    if (%TemplateData) {
        ...
        for my $Key (keys %FieldMapping) {
            $TemplateData{ $FieldMapping{$Key} } = $TemplateData{$Key};
            delete $TemplateData{$Key};
        }

        $TemplateData{Dest} = undef;
        if ( defined $TemplateData{QueueID} ) {
            my $QueueName = $Kernel::OM->Get('Kernel::System::Queue')->QueueLookup( QueueID => $TemplateData{QueueID} );
            if ( $QueueName ) {
                $TemplateData{Dest} = $TemplateData{QueueID} . '||' . $QueueName;
            } else {
                $TemplateData{QueueID} = 0;
                $TemplateData{Dest} = '||-';
            }
        }
        #print STDERR Dumper({%TemplateData});
